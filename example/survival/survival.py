...
train, test = train_test_split(
    data, test_size=0.2, stratify=stratifying_column, random_state=32
)
...
train_pool = Pool(
    train[features],
    label=train.loc[:, ["y_lower", "y_upper"]],
    cat_features=cat_features,
)
test_pool = Pool(
    test[features], label=test.loc[:, ["y_lower", "y_upper"]], cat_features=cat_features
)
...
test = [
    ["0", 60.0, "0", 20.4684, "0", "1", 52.0, "1", 84.0, 10.0, "0", "0", "0", 169.0],
